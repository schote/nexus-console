...
# TODO: Currently this contains normal cartesian SE sequence. Modify sequence to be a TSE returned by a function/constructor.
...
rf_refocussing = pp.make_sinc_pulse(
    flip_angle=pi, duration=rf_duration, system=system, use="refocusing"  # 180°  # keep duration -> doubles amplitude
)
...
grad_ro = pp.make_trapezoid(channel="x", system=system, flat_area=n_ro * delta_k, flat_time=adc_duration)
grad_ro_pre = pp.make_trapezoid(channel="x", system=system, area=grad_ro.area / 2 + delta_k / 2, duration=delay_te_1)

# Define adc event
adc = pp.make_adc(num_samples=n_ro, system=system, duration=adc_duration, delay=delay_te_2)
...
for pe_area in pe_area_values:
    ...
    grad_pe = pp.make_trapezoid(channel="y", system=system, area=pe_area, duration=delay_te_1)
