...
def write_acquisition_to_mrd(
    ...
    with ismrmrd.Dataset(dataset_path) as dataset:
        ...
        # Trajectory always has shape (3, num_ro), thus accounting the number of non-zero rows (axis=0),
        # yields the number of active gradients.
        # No gradient active -> n_dims = 0, RO + PE -> n_dims = 2, ...
        n_dims = sum(np.all(trajectory != 0, axis=1))
