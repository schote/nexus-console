...
class SequenceProvider(Sequence):
    ...
    def get_unrolled_grad(self, grad_block) -> list[np.array]:
        ...
        if grad_block.type == "grad":
            ...
            total_duration = grad_block.delay + grad_block.rise_time + grad_block.flat_time + grad_block.fall_time
            n_samples = int(total_duration * self.spcm_freq)
            waveform = [0., 0., grad_block.amplitude, grad_block.amplitude, 0.]
            time = [
                0., 
                grad_block.delay, 
